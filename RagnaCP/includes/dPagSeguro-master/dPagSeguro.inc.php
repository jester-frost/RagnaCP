...
class dPagSeguro{
	...
	Function newPagamento         ($pedido, $produtos, $callback=false, $nRetries=3){
		...
		$b->go("https://ws.pagseguro.uol.com.br/v2/checkout");
		$body     = $b->getBody();
		...
		$goUrl = "https://pagseguro.uol.com.br/v2/checkout/payment.html?code=".$response['code'];
		if($callback){
	...
	Function getNotification      ($type=false, $code=false){
		...
		if($type == 'transaction'){
			$b = new dBrowser2;
			$b->debug = $this->debug;
			$b->go("https://ws.pagseguro.uol.com.br/v2/transactions/notifications/{$code}?email={$this->settings['email']}&token={$this->settings['token']}");
			$body     = $b->getBody();
			$response = $this->_parseXML($body);
			if($response === false){
				$this->_addError("-999", "Resposta desconhecida: {$body}");
	...
	Function getTransaction       ($code){
		...
		$b = new dBrowser2;
		$b->debug = $this->debug;
		$b->go("https://ws.pagseguro.uol.com.br/v2/transactions/{$code}?email={$this->settings['email']}&token={$this->settings['token']}");
		$body     = $b->getBody();
		$response = $this->_parseXML($body);
